...
import FirebaseNotification from '../components/FirebaseNotification.vue';
const { getErrorMsg, getHasError, isPending } = storeToRefs(useAuthStore())
const notificationMsg = (getHasError == false ? 'Login successful!' : getErrorMsg)
...
const submit = () => {
    useAuthStore().login(user.email.value, user.password.value)
    if (isPending === false) {
        router.push('/')
    }
}
...
<template>
    <FirebaseNotification class="sticky top-0" :hasError="getHasError" :msg="notificationMsg"
        v-if="getErrorMsg != null" />
    <div class="max-w-screen-xl px-4 py-16 mx-auto sm:px-6 lg:px-8">
        <div class="max-w-lg mx-auto">
            ...
            <form @submit.prevent="submit" class="p-8 mt-6 mb-0 space-y-4 rounded-lg shadow-2xl text-gray-500">
                ...
                <button class="btn w-full btn-ghost loading text-sm normal-case font-medium" v-else>Signing in</button>
                <!-- forgot password link -->
                <div class="flex justify-between">
                    <router-link to="/forgot_password" class="text-sm text-indigo-600 hover:text-indigo-500">Forgot
                        password?</router-link>
                </div>

                <p class="text-sm text-center text-gray-500">
                    No account?
                    <router-link class="underline" to="/create_account">Sign up</router-link>
                </p>
